...
function Canned(dir, options) {
  ...
  this.dir = pathutil.resolve(process.cwd(), dir);
}
...
Canned.prototype.sanatizeContent = function (data, fileObject, httpObj) {
  var sanatized
  ...
  var tpl = fileObject.fname.match(new RegExp('.+\\.' + fileObject.mimetype.split('+')[0] + '(\\+(.+))?'));

  if(tpl) {
    data = template.process(fileObject.fname, data, tpl[2], httpObj.reqCtx);
  }
...
Canned.prototype._responseForFile = function (httpObj, files, cb) {
  ...
  if (fileObject) {
    ...
    fs.readFile(filePath, { encoding: 'utf8' }, function (err, data) {
      var response
      if (err) {
        response = new Response(getContentType('html'), '', 404, httpObj.res, that.response_opts, null, httpObj.headers)
        ...
        var content = that.sanatizeContent(data, fileObject, httpObj)

        if (content !== false) {
          response = new Response(_data.contentType || getContentType(fileObject.mimetype), content, statusCode, httpObj.res, that.response_opts, _data.customHeaders, httpObj.headers)
          cb(null, response)
        } else {
          content = 'Internal Server error invalid input file'
          response = new Response(getContentType('html'), content, 500, httpObj.res, that.response_opts, null, httpObj.headers)
          cb(null, response)
        }
      }
    })
  } else {
    var response = new Response(getContentType('html'), '', 404, httpObj.res, that.response_opts, null, httpObj.headers)
    cb('Not found', response)
  }
}
...
Canned.prototype.responder = function(body, req, res) {
  ...
  httpObj.reqCtx = {
    ...
    query: querystring.parse(httpObj.query),
    body: httpObj.content,
    path: parsedurl.pathname.slice(1).split('/')
  }
  ...
  if (httpObj.method === 'options') {
    ...
    var response = new Response(null, '', 200, res,  this.response_opts, null, httpObj.headers)
  ...
  var cpath = httpObj.pathname.join('/');
  ...
  responseHandler = function (err, resp) {
    if (err) {
      ...
      if (paths.length > 0) {
        ...
        that._log(' not found [' + cpath +'/'+ httpObj.dname +'.'+ httpObj.method +'.'+ httpObj.ctype + ']\n');
      }
    } else {
    ...
    fs.stat(httpObj.path + '/' + httpObj.dname, function (err, stats) {
      if (err) {
        ...
        if (stats.isDirectory()) {
          ...
          console.log('HHhhhhhhHHHHHHH', httpObj)
          cb(null, new Response('html', '', 500, httpObj.res, null, null, httpObj.headers))
        }
      }
    })
  })
...
Canned.prototype.responseFilter = function (req, res) {
  ...
  case 'POST':
    body = req.body || '';
    if(req.body) {
      that.responder(req.body, req, res)
    } else {
